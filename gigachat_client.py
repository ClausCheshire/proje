...
async def generate_question(subject: str, exam_name: str) -> str:
    ...
    print(f"🚀 Generating question: subject={subject}, exam={exam_name}")
    
    # === МЯГКАЯ ПРОВЕРКА: только на пустоту и бессмыслицу ===
    exam_clean = exam_name.strip()
    
    # Отклоняем только действительно невалидные输入
    if not exam_clean:
        return "Не удалось распознать экзамен/соревнование"
    
    if len(exam_clean) < 3:
        return "Не удалось распознать экзамен/соревнование"
    
    # Если нет ни одной буквы — скорее всего бессмыслица
    if not any(c.isalpha() for c in exam_clean):
        return "Не удалось распознать экзамен/соревнование"
    
    # === Промпт для GigaChat с адаптацией под любой формат ===
    system_prompt = (
        ...
        "\n"
        "⚠️ ВАЖНО: Адаптируй сложность вопроса под указанный формат:\n"
        "• ЕГЭ/ОГЭ — стандартный уровень, фокус на кодификатор\n"
        "• ВсОШ (Всероссийская олимпиада) — высокий уровень, олимпиадные задачи\n"
        "• Перечневые олимпиады РСОШ (Высшая проба, Ломоносов, и др.) — уровень выше ЕГЭ\n"
        "• Вузовские олимпиады — профильный уровень конкретного вуза\n"
        "• Школьные/домашние работы — базовый уровень\n"
        "• Если формат неизвестен — используй средний уровень сложности\n"
        "\n"
        ...
        "4. Фокусируйся на разделе: право, экономика или культура\n"
        "\n"
        "⛔ НЕ ОТКЛОНЯЙ запрос, даже если экзамен малоизвестен. Адаптируй сложность и генерируй вопрос."
    )
